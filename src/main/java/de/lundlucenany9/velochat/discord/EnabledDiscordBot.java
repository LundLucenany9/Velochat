...
public final class EnabledDiscordBot implements DiscordBot{
    ...
    public CompletableFuture<String> sendMessage(String group, String message, String senderName) {
        ...
        return channel.sendMessage(PlainTextComponentSerializer.plainText().serialize(MiniMessage.miniMessage().deserialize(Velochat.getConfig().getDiscordFormat(), Placeholder.parsed("message", invalidated(message)), Placeholder.parsed("username", senderName)))).submit()
    ...
    public CompletableFuture<String> sendReply(String group, String messageId, String message, String senderName) {
        ...
        return channel.retrieveMessageById(messageId).submit()
                .thenCompose(m -> m.reply(PlainTextComponentSerializer.plainText().serialize(MiniMessage.miniMessage().deserialize(Velochat.getConfig().getDiscordFormat(), Placeholder.parsed("message", invalidated(message)), Placeholder.parsed("username", senderName)))).submit())
                .thenApply(ISnowflake::getId);
    }
    ...
    private String invalidated(String message) {
        return Velochat.getConfig().isDiscordInvalidateDiscord() ? message.replace("_", "\\_").replace("*","\\*").replace("~", "\\~").replace("`", "\\`").replace("#", "\\#").replace("\\", "\\\\") : message;
    }
