                ...
                .then(BrigadierCommand.requiredArgumentBuilder("target", StringArgumentType.word())
                        ...
                        .then(BrigadierCommand.requiredArgumentBuilder("message", StringArgumentType.greedyString())
                                .executes(context -> {
                                    ...
                                    MessageFilterUtil.FilteredMessage filteredMessage = MessageFilterUtil.filter(context.getArgument("message", String.class), source);
                                    if(filteredMessage.shouldBlock()) return Command.SINGLE_SUCCESS;
                                    final String message = filteredMessage.finalMessage();
                                    ...
                                    proxy.getPlayer(targetName).ifPresentOrElse(target -> ReplyService.sendReply(source, target, message, ReplyRegistry.getLastBySender(target.getUniqueId())), () -> {
                                                String template = MessagesUtil.template(
                                                        Velochat.getMessages().player_not_online,
                                                        "<red>Player not online.</red>"
                                                );
